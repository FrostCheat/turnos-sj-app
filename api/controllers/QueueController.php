...
class QueueController {
    ...
    private function touchQueue(): void {
    ...
    public function getQueue(): array {
        ...
        if ($isAdmin) {
            $turns = $this->db->query("
                ...
                FROM turns t JOIN users u ON u.id = t.user_id
                ORDER BY FIELD(t.status,'active','waiting','completed','cancelled'),
            ...
            $myTurn = $this->db->prepare("
                ...
                FROM turns t WHERE t.user_id = ? AND t.status IN ('waiting','active')
    ...
    public function getTurn(): array {
        ...
        $stmt = $this->db->prepare("
            SELECT t.id, t.turn_number, t.service, t.status, t.position, t.notes,
                   t.created_at, t.called_at, t.completed_at,
                   u.id as user_id, u.name as user_name, u.email as user_email
            FROM turns t JOIN users u ON u.id = t.user_id WHERE t.id = ? LIMIT 1
        ");
        $stmt->execute([$id]);
        $turn = $stmt->fetch();
    ...
    public function getTurns(): array {
        ...
        $stmt = $this->db->prepare("
            SELECT t.id, t.turn_number, t.service, t.status, t.position, t.notes,
                   t.created_at, t.called_at, t.completed_at,
                   u.id as user_id, u.name as user_name, u.email as user_email
            FROM turns t JOIN users u ON u.id = t.user_id $whereStr
    ...
    public function createTurn(): array {
        ...
        if (!in_array($service, ALLOWED_SERVICES)) $service = 'Atención General';

        $existing = $this->db->prepare("SELECT id FROM turns WHERE user_id = ? AND status IN ('waiting','active') LIMIT 1");
        $existing->execute([$userId]);
        if ($existing->fetch()) return ['success' => false, 'message' => 'Ya tienes un turno activo o en espera.'];
        ...
        $stmt->execute([$userId, $max + 1, $service, $notes ?: null, $pos]);
        $id = (int)$this->db->lastInsertId();
        $this->touchQueue();

        return ['success' => true, 'message' => 'Turno creado correctamente.', 'id' => $id, 'turn_number' => $max + 1, 'position' => $pos];
    }
    ...
    public function updateTurn(): array {
        ...
        if (isset($body['service']) && $auth['role'] === 'admin') {
            ...
            if (in_array($svc, ALLOWED_SERVICES)) { $fields[] = 'service = ?'; $params[] = $svc; }
        }
        if (isset($body['notes'])) { $fields[] = 'notes = ?'; $params[] = trim($body['notes']) ?: null; }
        if (isset($body['status']) && $auth['role'] === 'admin') {
            if (in_array($body['status'], ['waiting','active','completed','cancelled'])) {
                ...
                if ($body['status'] === 'active') {
                    $fields[] = 'called_at = NOW()'; $fields[] = 'position = NULL';
                ...
                if (in_array($body['status'], ['completed','cancelled'])) {
                    $fields[] = 'completed_at = NOW()'; $fields[] = 'position = NULL';
                }
            }
        }
        ...
        if (in_array($body['status'] ?? '', ['completed','cancelled','waiting'])) $this->recalcPositions();
        $this->touchQueue();
    ...
    public function reorderQueue(): array {
        ...
        $body = json_decode(file_get_contents('php://input'), true) ?? [];
        $ids  = $body['ids'] ?? [];
        if (!is_array($ids) || empty($ids)) return ['success' => false, 'message' => 'IDs requeridos.'];

        $stmt = $this->db->prepare("UPDATE turns SET position = ? WHERE id = ? AND status = 'waiting'");
        foreach ($ids as $pos => $id) $stmt->execute([$pos + 1, (int)$id]);
